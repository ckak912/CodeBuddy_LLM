...
const model_role = `You have the role of a teaching assistant that is there to help students complete programming tasks. WYou only generate the next two lines of a given step.`
...
const next_step = `Loop through the grades array:
...
const model_prompt =`Given the current state of the user code, the step process provided, and the psuedo code for the current step, generate the next line of code that will help a student on the current step`
...
fetch(apiUrl, {
  ...
  .then(result => {
    ...
    const filePath = 'response_hint.txt';
